...
async fn ollama_call(client: &reqwest::Client, model: &str, prompt: String, json_mode: bool, max_tokens: i32) -> Result<String> {
    let req = OllamaRequest {
        model, prompt, stream: false,
        format: if json_mode { Some("json") } else { None },
        options: OllamaOptions { temperature: 0.2, num_predict: max_tokens },
    };
    let resp = client.post(OLLAMA_URL).timeout(Duration::from_secs(240)).json(&req).send().await
        .context("Ollama not reachable — is `ollama serve` running?")?;

    let status = resp.status();
    let text = resp.text().await.context("Failed to read Ollama response body")?;
...
#[derive(Deserialize)]
struct ScoringResult { relevance: f32, topic: String }
...
pub async fn score_articles(
    ...
    for chunk in articles.chunks_mut(5) {
        ...
        let _ = tx.send(ProgressEvent::Stage {
            stage: "SCORE".into(),
            message: format!("Scoring batch {}/{} ({} articles)…", batch_idx, n_batches, chunk.len()),
            percent,
        });

        let items: Vec<String> = chunk.iter().enumerate().map(|(i, a)| {
...
The array must have exactly {} entries in the same order as the items above."#,
        ...
        #[derive(Deserialize)]
        struct Wrapper { scores: Vec<ScoringResult> }
...
pub fn ollama_client() -> reqwest::Client {
    reqwest::Client::builder().timeout(Duration::from_secs(240)).build().expect("ollama client")
}
...
pub async fn check_ollama(model: &str) -> bool {
    ...
    let body: serde_json::Value = match resp.json().await { Ok(v) => v, Err(_) => return false };
    body.get("models").and_then(|m| m.as_array())
        .map(|arr| arr.iter().any(|m| m.get("name").and_then(|n| n.as_str()).map(|s| s.starts_with(model)).unwrap_or(false)))
        .unwrap_or(false)
}
